...
async function generateModelsData() {
  ...
  const outputPath = path.join(process.cwd(), "lib/ai/generated-models.ts");
...
`;
  ...
  console.log(`✅ Generated model data at: ${outputPath}`);
...
generateModelsData()
  .then(() => {
    console.log("✅ Model data generation complete!");
    process.exit(0);
  })
  .catch((error) => {
    console.error("❌ Failed to generate model data:", error);
    process.exit(1);
  });
