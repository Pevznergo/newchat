  ...
  try {
    ...
    if (response.toolCalls && response.toolCalls.length > 0) {
      const imageToolCall = response.toolCalls.find(
        ...
        // DALL-E 3 Generation Logic
        if (userType !== "pro") {
          // ... (keep existing pro check logic)
          await ctx.reply(
            "Для генерации изображений необходима PRO-подписка. 🔒\nВы можете купить её или попробовать выиграть в Колесе Фортуны!",
            {
               reply_markup: {
                 inline_keyboard: [
                   ...
                   [{ text: "Колесо Фортуны", web_app: { url: "https://app.aporto.tech/app" } }],
                 ],
               },
             }
           );
        ...
        const prompt = imageToolCall.args.prompt;
        await ctx.reply(`🎨 Генерирую изображение по запросу: "${prompt}"...`);

        try {
          // Using fetch directly for DALL-E 3
          ...
          if (imageUrl) {
             await ctx.replyWithPhoto(imageUrl, {
               caption: `🖼 ${prompt}\n\ngenerated by @aporto_bot`,
             });
          } else {
             await ctx.reply("Не удалось получить изображение. Попробуйте другой запрос.");
          }
        } catch (error) {
