...
function getGPTImagesKeyboard(aspectRatio = "1:1") {
...
function getImageModelKeyboard(selectedModel: string) {
  ...
  return {
    inline_keyboard: [
      ...
      [
        {
          text: `${isSelected("model_image_faceswap")}🎭 Замена лиц`,
          callback_data: "model_image_faceswap",
        },
        {
          text: `${isSelected("model_image_avatars")}📸 Набор аватарок`,
          callback_data: "model_image_avatars",
        },
      ],
      [
        {
          text: `${isSelected("model_image_upscale")}🔍 Увеличение X2/X4`,
          callback_data: "model_image_upscale",
        },
  ...
  return {
    inline_keyboard: [
      ...
      [
        ...
        {
          text: `${isSelected("model_video_kling_effects")}✨ Kling Effects 🆕`,
          callback_data: "model_video_kling_effects",
        },
      ],
      [
        {
        ...
        {
          text: `${isSelected("model_video_pika_effects")}💫 Pika Effects`,
          callback_data: "model_video_pika_effects",
        },
      ],
      [
        {
        ...
        {
          text: `${isSelected("model_video_pikaddition")}🧩 Pikaddition`,
          callback_data: "model_video_pikaddition",
        },
      ],
      [{ text: "Закрыть", callback_data: "menu_close" }],
    ],
  ...
  return {
    inline_keyboard: [
      [
        ...
        {
          text: `${isSelected("model_claude45sonnet")}Claude 4.5`,
      ...
      [
        {
          text: `${isSelected("model_grok41")}Grok 4.1`,
          callback_data: "model_grok41",
        },
        {
          text: `${isSelected("model_deepresearch")}Deep Research`,
...
🎭 Замена лица, повышение качества и другие сервисы 👇`;
  ...
  if (
    isSearchMenu &&
    (SEARCH_MODELS.includes(data) || data.startsWith("model_"))
  ) {
    const [user] = await getUserByTelegramId(telegramId);
    if (!user) {
      return;
  ...
  if (data === "/premium" || data === "/pro") {
    await ctx.answerCallbackQuery();
    await ctx.reply(
      "Для подключения Premium перейдите в раздел /premium (функционал в разработке)."
    );
    return;
  }
...
async function processTelegramMessage(
  ...
  const _telegramId = user.telegramId;
    ...
    messages: [
      {
        ...
        attachments, // Metadata for future reference (e.g. file_id)
        createdAt: new Date(),
      },
    ],
  });
  ...
  const aiMessages: any[] = history.map((m) => {
    ...
    const content = parts
      .map((p) => {
        ...
        if (p.type === "image") {
          return { type: "image", image: p.image }; // URL
        }
        return null;
      })
      .filter(Boolean);
  ...
  const modelId = user.selectedModel
    ? Object.keys(MODEL_NAMES).find((key) => key === user.selectedModel) ||
      "model_gpt4omini"
    : "model_gpt4omini";
  ...
  const realModelId = PROVIDER_MAP[modelId] || "openai/gpt-4o-mini";
...
💬 Есть вопросы? Пишите @GoPevzner`;
  ...
  if (text === "📝 Выбрать модель") {
    ...
    const _modelInfo = `В боте доступны ведущие модели ChatGPT, Claude, Gemini и DeepSeek:
(см. описание выше)
...
bot.on("message:photo", async (ctx) => {
  ...
  if (!photo) {
    await ctx.reply("Не удалось получить изображение. Попробуйте снова.");
    return;
  }
