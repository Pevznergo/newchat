...
export async function saveModel(data: FormData) {
    ...
    if (id) {
        await db.update(aiModel).set({
            modelId, name, providerId, type, description, isPremium, updatedAt: new Date()
        }).where(eq(aiModel.id, id));
    } else {
        await db.insert(aiModel).values({
            modelId, name, providerId, type, description, isPremium
        });
    }
...
export async function saveLimit(data: FormData) {
    ...
    const limitCount = parseInt(data.get("limitCount") as string, 10);
    const limitPeriod = data.get("limitPeriod") as string;
    // const isActive = data.get("isActive") === "on";

    if (id) {
        await db.update(modelLimit).set({
            limitCount, limitPeriod, updatedAt: new Date()
        }).where(eq(modelLimit.id, id));
    } else {
         await db.insert(modelLimit).values({
            modelId, userRole, limitCount, limitPeriod
        });
    }
