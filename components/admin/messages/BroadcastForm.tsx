...
export default function BroadcastForm({
	...
	const [formData, setFormData] = useState({
		...
		scheduledAt: initialData?.scheduledAt
			...
			: "",
		filters: {
			last_activity_days: initialData?.filters?.last_activity_days || "",
		},
	});
	...
	const handleCreate = async (type: "draft" | "start") => {
		...
		try {
			...
			const url = initialData
				? `/api/admin/messages/broadcasts/${initialData.id}`
				: "/api/admin/messages/broadcasts";
			...
			if (response.ok) {
				...
				if (type === "start") {
					...
					if (startResponse.ok) {
						...
						alert("Campaign saved but failed to start sending");
