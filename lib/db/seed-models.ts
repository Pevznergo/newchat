...
const MODELS = [
  ...
  },
  {
    ...
    providerId: "anthropic/claude-3-5-sonnet-20240620",
    type: "text",
    isPremium: true,
  },
  {
    modelId: "model_claude45thinking",
    name: "Claude 3.5 Thinking",
    providerId: "anthropic/claude-3-5-sonnet-20240620",
    type: "text",
    isPremium: true,
  },
  ...
  },
  {
    ...
    name: "Gemini 1.5 Pro",
    providerId: "google/gemini-1.5-pro-latest",
    type: "text",
    isPremium: true,
  },
  {
    ...
    providerId: "google/gemini-1.5-flash-latest",
    type: "text",
    isPremium: false,
  },
  {
    ...
    name: "Perplexity",
    providerId: "openrouter/perplexity/llama-3.1-sonar-large-128k-online",
    type: "text",
    isPremium: false,
  },
  {
...
async function seed() {
  ...
  for (const m of MODELS) {
    ...
    const existing = await db
      .select()
      .from(aiModel)
      .where(eq(aiModel.modelId, m.modelId));
    if (existing.length === 0) {
      ...
      if (existing[0].providerId !== m.providerId) {
        await db
          ...
          .where(eq(aiModel.id, existing[0].id));
