...
export async function processSuccessfulPayment({
    ...
    */
		...
		// 5. Update Clan Level (if user is in a clan)
		if (userRecord.clanId) {
			await refreshClanLevel(userRecord.clanId);
		}
...
import { calculateClanLevel } from "@/lib/clan/logic";
// getClanMemberCounts is defined in this file.

export async function refreshClanLevel(clanId: number) {
	try {
		// 1. Get current member counts
		const counts = await getClanMemberCounts(clanId);

		// 2. Get dynamic levels
		const dynamicLevels = await getClanLevels();

		// 3. Calculate new level
		const newLevel = calculateClanLevel(
			counts.totalMembers,
			counts.proMembers,
			dynamicLevels,
		);
		...
		// 4. Update Clan in DB
		// First get current level to check if changed?
		// Optimization: Just update.
		await db.update(clan).set({ level: newLevel }).where(eq(clan.id, clanId));

		console.log(`Refreshed clan ${clanId} level to ${newLevel}`);

		// 5. Optional: Reset/Update limits for all members if level changed?
		// The requirement says "limits are updated".
		// If we just update level, next `checkAndEnforceLimits` will pick up new config.
		// However, if `requestCount` or reset dates need adjustment, we'd do it here.
		// For now, limits are checked dynamically against `clanLevel` in `route.ts`.
		// BUT `checkAndEnforceLimits` in `route.ts` recalculates `clanLevel` on fly?
		// Let's check `route.ts` - it DOES `calculateClanLevel` inside!
		// So writing to DB `clan.level` is good for display, but `checkAndEnforceLimits` might be ignoring it?
		// `route.ts` L828:
		// clanLevel = calculateClanLevel(counts.totalMembers, counts.proMembers, CACHED_CLAN_LEVELS...);
		// So `route.ts` is ALREADY dynamic. Why did user say limits not updating?
		// Maybe `CACHED_CLAN_LEVELS` is stale?
		// Or maybe `checkAndEnforceLimits` is NOT used where they think?
		// Or maybe the display in /profile uses DB level, which is stale, confusing the user?
		// Yes, /profile L2028 uses `clanData.level` (from DB query?).
		// Fix: Updating DB level fixes /profile.

		return newLevel;
	} catch (error) {
		console.error(`Failed to refresh clan ${clanId} level`, error);
		return 1;
	}
}
